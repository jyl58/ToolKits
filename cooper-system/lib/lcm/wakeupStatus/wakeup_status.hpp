...
#ifndef __wakeupStatus_wakeup_status_hpp__
#define __wakeupStatus_wakeup_status_hpp__

#include "lcm_coretypes.h"

#include <string>

namespace wakeupStatus
{
...
int wakeup_status::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int wakeup_status::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int wakeup_status::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t wakeup_status::getHash()
{
    static int64_t hash = static_cast<int64_t>(_computeHash(NULL));
    return hash;
}

const char* wakeup_status::getTypeName()
{
    return "wakeup_status";
}

int wakeup_status::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int wakeup_status::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
...
int wakeup_status::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t wakeup_status::_computeHash(const __lcm_hash_ptr *)
{
    uint64_t hash = 0xa390a7982d9682ddLL;
